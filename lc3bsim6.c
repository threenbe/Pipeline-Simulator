...
int target_pc, trap_pc, mem_drid, mem_pc_mux, v_mem_ld_cc, v_mem_ld_reg;
/*v_mem_br_stall, mem_stall*/
...
void MEM_stage() {
	...
	int read_word = 0;
	int dcache_r = 0;
	int dcache_enable_bit = Get_DCACHE_EN(PS.MEM_CS) & PS.MEM_V;
	NEW_PS.SR_DATA = 0;/*if DCACHE_EN == 0*/

	if (dcache_enable_bit) {
		if (Get_DCACHE_RW(PS.MEM_CS)) {
			if (!Get_DATA_SIZE(PS.MEM_CS)) {/*byte*/
				if(!(PS.MEM_ADDRESS & 0x1)) {
					...
					WE1 = 1;
					write_word = Low16bits((PS.MEM_ALU_RESULT << 8) + (PS.MEM_ALU_RESULT & 0xFF));
				}
			}
			if (Get_DATA_SIZE(PS.MEM_CS) && !(PS.MEM_ADDRESS & 0x1)) {/*word*/
				WE0 = 1;
				WE1 = 1;
				write_word = Low16bits(PS.MEM_ALU_RESULT);
			}
		...
		dcache_access(Low16bits(PS.MEM_ADDRESS), &read_word, write_word, &dcache_r, WE0, WE1);
		...
		if (dcache_r == 0) {
			...
			if (!Get_DCACHE_RW(PS.MEM_CS)) {
				if (!Get_DATA_SIZE(PS.MEM_CS)) {
					if (!(PS.MEM_ADDRESS & 0x1)) {
						read_word = Low16bits(sext(read_word & 0xFF, 8));
					} else {
						read_word = Low16bits(sext((read_word >> 8) & 0xFF, 8));
					}
				}/*else read_word = read_word*/
				NEW_PS.SR_DATA = Low16bits(read_word);
			}
		}
	}
		...
		mem_stall = 0;/*not sure; doc says assert if valid AND needs to access memory (and dcache_r=0), and this doesn't access memory so...*/
	}
	...
	int nzp_bits = (PS.MEM_IR >> 9) & 0x7;
	if (PS.MEM_V) {
		if(Get_TRAP_OP(PS.MEM_CS)) {
			mem_pc_mux = 2;
		} else if ((Get_BR_OP(PS.MEM_CS) && (nzp_bits & PS.MEM_CC)) || Get_UNCOND_OP(PS.MEM_CS)) {
			mem_pc_mux = 1;
		} else {
			mem_pc_mux = 0;
		}
...
void AGEX_stage() {
	...
	switch(Get_ADDR1MUX(PS.AGEX_CS)) {
		case 0: 
		addr1Output = Low16bits(PS.AGEX_NPC);
		break;

		case 1:
		addr1Output = Low16bits(PS.AGEX_SR1);
		break;
	}
	...
	switch(Get_ADDR2MUX(PS.AGEX_CS)) {
		...
		case 1:
		addr2Output = Low16bits(sext((PS.AGEX_IR & 0x3F), 6));
		break;

		case 2:
		addr2Output = Low16bits(sext((PS.AGEX_IR & 0x1FF), 9));
		break;

		case 3:
		addr2Output = Low16bits(sext((PS.AGEX_IR & 0x7FF), 11));
		break;
	}

	if (Get_LSHF1(PS.AGEX_CS)) {
		addr2Output = addr2Output << 1;
	}

	int addressmuxOutput;

	switch (Get_ADDRESSMUX(PS.AGEX_CS)) {
		case 0: /* select LSHF(ZEXT[IR[7:0]],1) */
		addressmuxOutput = Low16bits((PS.AGEX_IR & 0xFF) << 1);
		break;

		case 1: /*select adder output*/
		addressmuxOutput = Low16bits(addr1Output + addr2Output);
		break;
	}
	...
	switch(Get_SR2MUX(PS.AGEX_CS)) {
		...
		case 1: /*imm5*/
		SR2 = Low16bits(sext((PS.AGEX_IR & 0x1F), 5));
		break;
	}
	...
	switch(Get_ALUK(PS.AGEX_CS)) {
		...
		case 3: /*PASSB*/
		aluOutput = Low16bits(SR2);
		break;
	}

	int shiftAmt = PS.AGEX_IR & 0xF;
	int shiftType = (PS.AGEX_IR >> 4) & 0x3;
	int shiftOutput = 0;

	switch(shiftType) {
		case 0: /* LSHF */
		shiftOutput = Low16bits(SR1 << shiftAmt);
		break;
		...
		case 2: /* still a left shift LUL */
		shiftOutput = Low16bits(SR1 << shiftAmt);
		break;

		case 3: /* RSHFA */
		shiftOutput = Low16bits(sext(SR1 >> shiftAmt, 16 - shiftAmt));
		break;
	}
	...
	if (LD_MEM) {
		...
		NEW_PS.MEM_ADDRESS = addressmuxOutput;
...
void DE_stage() {
	...
	CONTROL_STORE_ADDRESS = ((PS.DE_IR & 0xF800) >> 10) | ((PS.DE_IR >> 5) & 0x1);

	/* your code for DE stage goes here */

  	int SR1ID = (PS.DE_IR >> 6) & 0x7;
  	int SR2ID;
  	int DRID;

  	switch((PS.DE_IR >> 13) & 0x1) {
  		case 0:
  		SR2ID = PS.DE_IR & 0x7;
  		break;

  		case 1:
  		SR2ID = (PS.DE_IR >> 9) & 0x7;
  		break;
  	}
  	...
  	int agexSR1 = REGS[SR1ID];
  	int agexSR2 = REGS[SR2ID];

  	if (v_sr_ld_reg) { /*write to register file*/
  	...
  	/*dep_stall is asserted if the instruction in DE stage is valid AND 
  	(at least one of its input values haven't been written into the reg file yet OR the instruction is a conditional branch and cc's are being set in later stages)*/

  	if (PS.DE_V) {
  		if ( ( SR1_needed && v_agex_ld_reg && (SR1ID == agex_drid) )
  			|| ( SR1_needed && v_mem_ld_reg && (SR1ID == mem_drid) ) 
  			|| ( SR1_needed && v_sr_ld_reg && (SR1ID == sr_reg_id) ) 
  			|| ( SR2_needed && v_agex_ld_reg && (SR2ID == agex_drid) ) 
  			|| ( SR2_needed && v_mem_ld_reg && (SR2ID == mem_drid) ) 
  			|| ( SR2_needed && v_sr_ld_reg && (SR2ID == sr_reg_id) ) 
  			|| ( de_br_op && (v_mem_ld_cc || v_agex_ld_cc || v_sr_ld_cc) ) ) {
  	...
  	if (PS.DE_V) {
  		v_de_br_stall = Get_DE_BR_STALL(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
  	} else {
  		v_de_br_stall = 0;
  	}
	...
	if (LD_AGEX) {
    	...
    	NEW_PS.AGEX_SR1 = agexSR1;
    	NEW_PS.AGEX_SR2 = agexSR2;
    	NEW_PS.AGEX_CC = (agexN << 2) | (agexZ << 1) | (agexP);
...
void FETCH_stage() {
	...
	int LD_PC, LD_DE, DE_valid, DE_next_pc, DE_instr;

	DE_next_pc = Low16bits(PC+2);

	icache_access(PC, &DE_instr, &icache_r);

	int stall_logic = !icache_r || dep_stall || v_de_br_stall || v_agex_br_stall || v_mem_br_stall || mem_stall;
	LD_PC = !stall_logic || (v_mem_br_stall && (mem_pc_mux != 0));
	LD_DE = !mem_stall && !dep_stall;
	/*If icache_r is not asserted and pipeline is not stalling for some other reason, DE_V = 0
	if the pipeline is stalling somewhere down the line then also DE_V = 0 I think
	Dunno if we need dep_stall and mem_stall for this?*/
	DE_valid = (icache_r) && !(v_de_br_stall || v_agex_br_stall || v_mem_br_stall);
	...
	if (LD_DE) {
		NEW_PS.DE_NPC = DE_next_pc;
		NEW_PS.DE_IR = DE_instr;
